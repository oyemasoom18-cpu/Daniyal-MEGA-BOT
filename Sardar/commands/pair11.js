...
const templateUrl = "https://i.ibb.co/G4zM6y6b/1ed37743aeb2.jpg";
const templatePath = path.join(cacheDir, "pair11_template.png");
...
module.exports = {
  config: {
    credits: "SARDAR RDX",
    name: "pair11",
    aliases: ["couple11", "pinkpair", "iloveyou"],
    description: "Pink Love pair image banao — Circle style (Style 11).",
    usage: "pair11 [@mention / reply]",
    category: "Love",
    prefix: true,
    cooldowns: 5
  },
...
` +
    ...
    const { msgID: loadMsgID, done: animDone } = await animateProgress(api, threadID, messageID, '💗', '𝐏𝐢𝐧𝐤 𝐋𝐨𝐯𝐞', 'pair11');
    ...
    try {
      ...
      const outputPath = path.join(cacheDir, `pair11_${one}_${two}_${Date.now()}.png`);
