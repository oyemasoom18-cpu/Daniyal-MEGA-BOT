...
const templateUrl = "https://i.ibb.co/qL1MMy0D/0fa8f49ebd54.jpg";
const templatePath = path.join(cacheDir, "sister3_template.jpg");
...
module.exports = {
  config: {
    credits: "SARDAR RDX",
    name: "sister3",
    aliases: ["sis3", "sistersforever3", "wingsister"],
    description: "Sister pair image banao — Sisters Forever Wings style (Sister 3).",
    usage: "sister3 [@mention / reply]",
    category: "Love",
    prefix: true,
    cooldowns: 5
  },
...
` +
    ...
    const { msgID: loadMsgID, done: animDone } = await animateProgress(api, threadID, messageID, '💗', '𝐒𝐢𝐬𝐭𝐞𝐫𝐬 𝐅𝐨𝐫𝐞𝐯𝐞𝐫', 'sister3');
    ...
    try {
      ...
      const outputPath = path.join(cacheDir, `sister3_${one}_${two}_${Date.now()}.png`);
