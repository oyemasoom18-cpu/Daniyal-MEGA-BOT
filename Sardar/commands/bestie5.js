...
const templateUrl = "https://i.ibb.co/G3ccWwCR/b041d6c8c51d.jpg";
const templatePath = path.join(cacheDir, "bestie5_template.jpg");
...
module.exports = {
  config: {
    credits: "SARDAR RDX",
    name: "bestie5",
    aliases: ["bff5", "bestfriend5", "melodybesties"],
    description: "My Melody Besties pair image banao — Butterfly & Heart style (Bestie 5).",
    usage: "bestie5 [@mention / reply]",
    category: "Love",
    prefix: true,
    cooldowns: 5
  },
...
` +
    ...
    const { msgID: loadMsgID, done: animDone } = await animateProgress(api, threadID, messageID, '🩷', '𝐁𝐞𝐬𝐭𝐢𝐞 𝐏𝐚𝐢𝐫', 'bestie5');
    ...
    try {
      ...
      const outputPath = path.join(cacheDir, `bestie5_${one}_${two}_${Date.now()}.png`);
